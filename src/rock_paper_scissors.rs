...
fn ai_move (v:[usize; 3]) -> Choice { //weighted random choice
...
fn main() {
    ...
    loop {
        ...
        match input.to_lowercase().trim() {
            s if s.starts_with("r") => {uchoice = Rock; ucf[0]+=1},
            s if s.starts_with("p") => {uchoice = Paper; ucf[1]+=1},
            s if s.starts_with("s") => {uchoice = Scissors; ucf[2]+=1},
        ...
        println!("You chose {}, I chose {}.",
            uchoice.print(), aichoice.print());
        if beats(uchoice, aichoice) {
            score[0] += 1;
            println!("You win!");
        } else
            if uchoice == aichoice { println!("It's a tie!");
            } else {
                score[1] += 1;
                println!("I win!");
                }
