...
enum MatchState {
    ...
    Ci,         //Last seen a 'c' followed by an 'i'
    E,          //Last seen an 'e' not preceded by a 'c'
    I           //Last seen an 'i' not preceded by a 'c'
}
    ...
    for c in data.chars() {
        state = match (state, c.to_lowercase()) {
            (_, '\n') | (_, '\r') => {
                ...
                if saw_ei  {result.ei += 1;  saw_ei = false;}
                MatchState::Nothing
            },
            (_, 'c') => MatchState::C,
            (MatchState::C, 'i') => MatchState::Ci,
            (MatchState::Ce, 'i') => {saw_cei = true; MatchState::Nothing},
            (MatchState::E, 'i') => {saw_ei = true; MatchState::Nothing},
            (_, 'i') => MatchState::I,
            (MatchState::C, 'e') => MatchState::Ce,
            (MatchState::Ci, 'e') => {saw_cie = true; MatchState::Nothing},
            (MatchState::I, 'e') => {saw_ie = true; MatchState::Nothing},
            (_, 'e') => MatchState::E,
