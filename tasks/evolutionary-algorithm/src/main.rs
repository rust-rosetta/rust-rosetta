...
fn main() {
    let target = "methinks it is like a weasel"; // lowercase for readability
    let copies = 100;
...
fn fitness(target: &str, sentence: &str) -> usize {
    let zip = sentence.chars().zip(target.chars());
    zip.filter(|&(c1, c2)| c1 != c2).count()
}
...
/// Generates a random lowercase letter or space.
fn random_char<R: Rng>(rng: &mut R) -> char {
    // Returns a value in the range [a, z] + an extra slot for the space character.  '|' is the
    // exclusive upper bound.  (The `u8` values could be cast to larger integers for a better
    // chance of the RNG hitting the proper range).
    match rng.gen_range(b'a', b'|') as char {
        '{' => ' ', // the `char` after 'z'
        c => c,
    }
}
